//chb: can't include interface for access to game print functions b/c of circular dependency
//#include "interface.h"
#include "dominion.h"
...
int whosNextPlayer(struct gameState *state)
{
  int currentPlayer = whoseTurn(state);
  return state->numPlayers - 1 ? 0 : currentPlayer + 1;
}
...
int playBaron( struct gameState *state, int choice1)
{
  ...
  if (choice1 > 0)
  {
    ...
    while(card_not_discarded)
    {
      ...
      {
        //chb: a finite amount of copper in game--why is next call not subtracting from it? 
        state->coins += 4;//Add 4 coins to the amount of coins
        
        /*chb: DEBUG: this will have to wait until we can refactor print* functions in interface
         * and move them to a separate source file to avoid cyclical dependencies 
        printf("Printing hand: \n");
        printHand(currentPlayer, state);
        printf("Printing discard pile: \n");
        printDiscard(currentPlayer, state);
        //chb: end DEBUG */ 
        
        //chb: add to discard pile the estate card just encountered 
        ...
        //chb: BARBUG1: don't decrement handCount after discard
        //state->handCount[currentPlayer]--;
        
        /*chb: DEBUG: see comment above
        printf("Printing hand: \n");
        printHand(currentPlayer, state);
        printf("Printing discard pile: \n");
        printDiscard(currentPlayer, state);
  ...
  else
  {
    if (supplyCount(estate, state) > 0)
    {
      ...
      if (supplyCount(estate, state) == 0)
      {
        //chb: BARBUG2: don't check for gameover
        //isGameOver(state);
      }
    }
  }
...
int playMinion( struct gameState *state, int choice1, int choice2, int handPos)
{
  ...
  {
    //discard hand
    //chb: MIOBUG1: inequality should be > 0, not > 1 
    ...
    for (int x = 0; x < state->numPlayers; x++)
    {
      if (x != currentPlayer)
      {
        if ( state->handCount[x] > 4 )
        {
          //discard hand
          while( state->handCount[x] > 0 )
          {
            discardCard(handPos, x, state, 0);
          }

          //draw 4
          //chb: MIOBUG2: limiting condition should be 4, not 5--players get one card too many
          for (int j = 0; j < 5; j++)
          {
            drawCard(x, state);
          }
...
int playAmbassador(struct gameState *state, int choice1, int choice2, int handPos)
{
  ...
  //choice2 is the number of cards of type choice1 the player wants to discard
  ...
  {
    //chb: AMBBUG1: removing last boolean clause/term from conditional 
    //  (was `if (i != handPos && i == state->hand[currentPlayer][choice1] && i != choice1)` )
  ...
  for (int i = 0; i < state->numPlayers; i++)
  {
    if (i != currentPlayer)
    {
      //chb: AMBBUG2: each other player gains a copy of the wrong card 
      //  (was `gainCard(state->hand[currentPlayer][choice1], state, 0, i);`)
      gainCard(state->hand[currentPlayer][0], state, 0, i);
    }
  }
...
{
  int currentPlayer = whoseTurn(state);
  int nextPlayer = whosNextPlayer(state);

  int tributeRevealedCards[2] = {-1, -1};
  
  //chb: checks least likely scenario--user has 1 or 0 cards in both piles--first?! 
  //chb: TRIBUG1: was `if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1)`
  if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 2)
  {
  ...
  {
    //chb: if deck is empty, we must get our two cards from the discard pile
    if (state->deckCount[nextPlayer] == 0)
    {
      ...
      //chb: TRIBUG2: error to not shuffle deck (also, as noted in Assignment 1, the shuffle call 
      //  should be made _before_ the for loop)
      //shuffle(nextPlayer,state);//Shuffle the deck
    } 
    //chb: otherwise, we must use the deck
    tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
...
int playMine(struct gameState *state, int choice1, int choice2, int handPos)
{
  ...
  //chb: MINBUG1: was `int j = state->hand[currentPlayer][choice1];`
  ...
  //chb: MINBUG2: was `if ( (getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2))`
